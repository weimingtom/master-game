 ...
 * - T_MAP - value of nth key/value pair in the map;
 * - T_FUNCTION - value of nth argument of the function;
 * If the VALUE is not of one of types above then iPath
hsmile/win32/win32_graphin.cpp
t makes it of type T_ARRAY with 
...
EXTERN_C UINT VALAPI ValueInvoke( VALUE* pval, VALUE* pthis, UINT argc, const VALUE* argv, VALUE* pretval, LPCWSTR url);
  ...
  namespace json
  {
    ...
    //inline const value_key_a 
    //    value::operator[](const value& key) const { return value_key_a(*const_cast<value*>(this), key); }
    inline value_key_a 
    ...
    //inline const value_idx_a 
    //    value::operator[](int idx) const { return value_idx_a(*const_cast<value*>(this), idx); }
    inline value_idx_a 
